...
const UnifiedNewUserPage: React.FC = () => {
  const navigate = useNavigate();
  const { addUser } = useUnifiedUsers();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      // Adicionar usuário
      const addedUser = addUser(newUser, formData.senha);

      if (!addedUser) {
        setError('Erro ao adicionar usuário. Verifique se o email já está em uso.');
        return;
      }

      // Mostrar mensagem de sucesso
      setSuccess(`Usuário ${newUser.nome} criado com sucesso!`);
...
export default UnifiedNewUserPage;

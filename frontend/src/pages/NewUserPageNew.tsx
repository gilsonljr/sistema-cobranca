...
const NewUserPageNew: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      // Converter para o formato User e adicionar usando o novo UserStore
      const newUser = convertToUserType(legacyUser);
      addUser(newUser);

      // Salvar a senha no UserPasswordService
      const passwordsStr = localStorage.getItem('user_passwords');
      const passwords = passwordsStr ? JSON.parse(passwordsStr) : {};
      passwords[formData.email.toLowerCase()] = formData.senha;
      localStorage.setItem('user_passwords', JSON.stringify(passwords));

      // Atualizar default_users para garantir compatibilidade com o sistema de login
      const defaultUsersStr = localStorage.getItem('default_users');
      const defaultUsers = defaultUsersStr ? JSON.parse(defaultUsersStr) : {};
      defaultUsers[formData.email.toLowerCase()] = {
        id: newId,
        email: formData.email.toLowerCase(),
        fullName: formData.nome,
        role: papeis.includes('admin') ? 'admin' :
              papeis.includes('supervisor') ? 'supervisor' :
              papeis.includes('operador') ? 'collector' :
              papeis.includes('vendedor') ? 'seller' : 'user',
        password: formData.senha
      };
      localStorage.setItem('default_users', JSON.stringify(defaultUsers));

      console.log('Usuário criado usando o novo UserStore e senhas sincronizadas:', newUser);
